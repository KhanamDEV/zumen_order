                            ...
                            <table class="table table-bordered" id="table-project">
                                ...
                                @foreach($data['projects']['list'] as $key => $project)
                                    ...
                                    >
                                        <td class="index" >
                                            <a href="#">{{$key +  1}}</a></td>
                                        <td><a href="{{!empty($project->project_id) ? route('admin.project.feedback.detail', ['id' => $project->id, 'project_id' => $project->project_id]) : route('admin.project.show', ['id' => $project->id])}}">{{@$project->owner}}</a></td>
    ...
    <script>
        $(document).ready( function () {
            $("#analytics-project").change(function (){
                ...
                let year = $(this).val();
                $.ajax({
                    url: '{{route('admin.project.analytics_by_year')}}?year=' + year,
                    method: 'GET',
            ...
            $.ajax({
                url: '{{route('admin.project.analytics_by_year')}}?year=' + '{{date('Y')}}',
                method: 'GET',
                success: function (res){
                    res = res.response;
                    Object.keys(res.amount).forEach(function (item){
