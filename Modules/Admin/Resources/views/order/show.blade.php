    ...
    <section class="content">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <p class="info"><span>納品日</span>: {{@$order->project->delivery_date}}</p>
                    </div>
                    <div class="col-md-4">
                ...
                @php
                    $readonly = strtotime(date('Y-m-d')) > strtotime(date('Y-m-d', strtotime($order->project->delivery_date))) || $order->status == 5;
                    @endphp
                <form action="" method="POST">
                    ...
                    @if(!$readonly)
                    <div class="group-button mt-3 text-right">
                        <button type="submit" class="btn btn-success mr-2">送信</button>
                        <a href="{{route('worker.order.leave_project', ['id' => $order->id])}}" class="btn btn-danger">プロジェクトを離れる</a>
                    </div>
                        @endif
                </form>
            </div>
