...
class SendMailCompleteProject implements ShouldQueue
{
                           ...
                           AdminRepositoryInterface  $adminRepository, UserRepositoryInterface $userRepository)
    {
        ...
        foreach ($workers as $worker){
            $job = new SendMailCompleteProjectSingle('worker', $this->order, $worker->email);
            dispatch($job)->delay(now()->addSeconds(4));
        }
        foreach ($users as $user){
            $job = new SendMailCompleteProjectSingle('user', $this->order, $user->email);
            dispatch($job)->delay(now()->addSeconds(4));
        }
    }
}
