...
class SendMailCancelProject implements ShouldQueue
{
...
//        }
        foreach ($workers as $worker){
            $job = new SendMailCancelProjectSingle('worker', $this->order, $worker->email);
            dispatch($job)->delay(now()->addSeconds(4));
        }
        foreach ($users as $user){
            $job = new SendMailCancelProjectSingle('user', $this->order, $user->email);
            dispatch($job)->delay(now()->addSeconds(4));
        }
    }
}
