...
class SendMailLeaveProject implements ShouldQueue
{
                           ...
                           AdminRepositoryInterface  $adminRepository, UserRepositoryInterface $userRepository)
    {
        ...
        foreach ($admins as $admin){
            $job = new SendMailLeaveProjectSingle('admin', $this->order, $admin->email);
            dispatch($job)->delay(now()->addSeconds(4));
        }
        foreach ($workers as $worker){
            $job = new SendMailLeaveProjectSingle('worker', $this->order, $worker->email);
            dispatch($job)->delay(now()->addSeconds(4));
        }
        foreach ($users as $user){
            $job = new SendMailLeaveProjectSingle('user', $this->order, $user->email);
            dispatch($job)->delay(now()->addSeconds(4));
        }
    }
}
