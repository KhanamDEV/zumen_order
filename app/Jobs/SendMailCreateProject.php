...
class SendMailCreateProject implements ShouldQueue
{
...
//        $users = $userRepository->getList([]);
        foreach ($admins as $admin){
            $job = new SendMailCreateSingleProject('admin', $this->order, $admin->email);
            dispatch($job)->delay(now()->addSeconds(4));
        }
        foreach ($workers as $worker){
            $job = new SendMailCreateSingleProject('worker', $this->order, $worker->email);
            dispatch($job)->delay(now()->addSeconds(4));
        }
//        foreach ($users as $user){
//            $job = new SendMailCreateSingleProject('user', $this->order, $user->email);
//            dispatch($job)->delay(now()->addSeconds(4));
//        }
    }
}
